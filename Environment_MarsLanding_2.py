...
WIND_SPEED_RANGE = np.array([0, 5]) / SCREEN_WIDTH
WIND_SPEED_SHIFT = 1 / SCREEN_WIDTH
...
class RocketEnv(gym.Env):
    ...
    def __generate_relief(self):
        ...
        pY = 0
        shift = 25
        coords = [(pX, pY), (pX, random.randint(int(0.667*SCREEN_HEIGHT), SCREEN_HEIGHT-shift))]

        # The relief to the left to a plato
        left_points = random.choice(range(3,8))
        ...
        for i, c in enumerate(range(left_points)):
            ...
            if c == left_points - 1:
                pY = random.randint(shift, 125)
            else:
                pY = random.randint(shift, upper_bound)
            coords.append((pX, pY))
        ...
        right_points = random.choice(range(3,8))
        right_bound = int(SCREEN_WIDTH) - shift
        for i in range(right_points):
            pX = random.randint(pX, right_bound)
            pY = random.randint(shift, upper_bound)
            coords.append((pX, pY))

        coords.extend([(SCREEN_WIDTH, random.randint(int(0.667*SCREEN_HEIGHT), SCREEN_HEIGHT-shift)), (SCREEN_WIDTH, 0)])
        return coords
    ...
    def _render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            # Relief
            self.relief_coords = [(0, 0), (0, 300), (300, 50), (600, 300), (600, 0)]
        ...
        rockety = (1.0 - states[1])*ROCKET_HEIGHT + ROCKET_HEIGHT / 2.0  # MIDDLE OF CART
